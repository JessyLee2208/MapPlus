...
function renderStar(data, newArray) {
  ...
  if (data !== 0) {
    ...
    for (let i = 0; i < length; i++) {
      let radomNumber = Math.random().toString(36).substr(10, 8) + Date.now().toString(36).substr(4, 4);
    ...
    if (data % length !== 0) {
      ...
      for (let i = 0; i < 4 - length; i++) {
        let radomNumber = Math.random().toString(36).substr(10, 6) + Date.now().toString(36).substr(4, 4);
